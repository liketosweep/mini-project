...
export function RoomCheckInCard({
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-zinc-100 dark:border-zinc-800 pb-4">
        ...
        <div>
          {isTodayCheckedIn ? (
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-800 border border-amber-200 dark:bg-amber-950/40 dark:border-amber-900 dark:text-amber-300">
      ...
      {!isEligible ? (
        <div className="rounded-xl border border-dashed border-zinc-200 bg-zinc-50/70 p-4 text-center dark:border-zinc-800 dark:bg-zinc-800/30 space-y-1.5">
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 p-5 dark:border-emerald-950/50 dark:bg-emerald-950/20 space-y-3">
          ...
          {todayLog?.note && (
            <div className="rounded-lg bg-white/80 dark:bg-zinc-900/80 p-3 border border-emerald-100 dark:border-emerald-900/40 text-xs text-zinc-700 dark:text-zinc-300 flex items-start gap-2">
        ...
        <form onSubmit={handleCheckIn} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full rounded-xl border border-zinc-300 px-3.5 py-2.5 text-xs text-zinc-900 placeholder:text-zinc-400 focus:border-indigo-600 focus:outline-none focus:ring-1 focus:ring-indigo-600 dark:border-zinc-700 dark:bg-zinc-800 dark:text-white dark:placeholder:text-zinc-500 disabled:opacity-50 resize-none"
            />
          </div>
          ...
          <div className="flex items-center justify-end">
            <button
              type="submit"
              disabled={isSubmitting}
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:ring-offset-2 disabled:opacity-50 transition-colors"
            >
              {isSubmitting ? (
                <>
